...
<html>
	...
	<body>
		...
		<div class="nav text-center">
			...
			<span class="font16">货运审核</span>
			<?php if($info['auditType'] == 3): else: ?>
			<button class="mybtn pull-right font15 compile" type="button" id="goodcheck">
				保存
			</button><?php endif; ?>
		...
		<div class="container-fluid cont">
			<div class="row">
				<div class="col-xs-12">
					<p class="pad-up15 mar-b0">
						<img class="check" src="/Public/img/qtg.png"/>
						...
						<?php elseif($info['auditType'] == 4): ?><span class="font15">未通过审核</span>
						<?php else: endif; ?>
					</p>
				</div>
			</div>
			...
			<div class="row mar-t15">
				<div class="col-xs-12" id="imageaqa">
					<button type="button" class="mybtn">
					 <?php if($info['idCardImage'] == '' ): ?><img class="tj" src="/Public/img/tianjia.png"  id="imagess"/>
					<font size="1" style="margin-left:20px;">图例&nbsp;&nbsp;&nbsp;&nbsp;<img  src="/Public/img/tosan.jpg"  class="tj"/></font>
					<?php else: ?>
					<img class="tj" src="<?php echo ($info["idCardImage"]); ?>"    id="imagess"/><?php endif; ?>
					 </button>
				</div>
			...
			<div class="row mar-t15">
				<div class="col-xs-12" id="imageaqa1">
					<button type="button" class="mybtn"> 
						 <?php if($info['vehicleLicense'] == '' ): ?><img class="tj" src="/Public/img/tianjia.png"  id="imagess1"/>
					<font size="1" style="margin-left:20px;">图例&nbsp;&nbsp;&nbsp;&nbsp;<img  src="/Public/img/tosan1.jpg"  class="tj"/></font>
					<?php else: ?>
					<img class="tj" src="<?php echo ($info["vehicleLicense"]); ?>"  id="imagess1"/><?php endif; ?>
					</button>
				</div>
			...
			<div class="row mar-t15">
				<div class="col-xs-12"  id="imageaqa2">
					<button type="button" class="mybtn"> 
						 <?php if($info['reserve1'] == '' ): ?><img class="tj" src="/Public/img/tianjia.png"  id="imagess2"/>
					<font size="1" style="margin-left:20px;">图例&nbsp;&nbsp;&nbsp;&nbsp;<img  src="/Public/img/tosan2.jpg"  class="tj"/></font>
					<?php else: ?>
					<img class="tj" src="<?php echo ($info["reserve1"]); ?>"  id="imagess2"/><?php endif; ?>
					</button>
				</div>
			...
			<div class="row bg-fff mar-t15">
				<div class="col-xs-12 pad-up10 font15 bor-b-f2">
					<label>车牌号</label><input class="inp" type="text" id="carnumber" placeholder="请输入车牌号" value="<?php echo ($info["licensePlate"]); ?>"/>
				</div>
				<div class="col-xs-12 pad-up10 font15 bor-b-f2">
					<label>车辆类型</label>
					<select name="" id="cartype">
				    <?php if($info['vehicleType'] == 1): ?><option value="1">面包车</option>
					<?php elseif($info['vehicleType'] == 2): ?>
					        <option value="2">小型厢货</option>
				    <?php elseif($info['vehicleType'] == 3): ?>
					        <option value="2">大型厢货</option>
							<?php else: endif; ?>
					<option value="1">面包车</option>
					<option value="2">小型厢货</option>
					<option value="3">大型厢货</option>
				</select>
					
					<!--<input class="inp" type="text" id="cartype" placeholder="请输入车辆类型" value="<?php echo ($info["vehicleType"]); ?>"/>-->
				</div>
				<div class="col-xs-12 pad-up10 font15 bor-b-f2">
					<label>车辆详情</label><font color="red" size="1" style="margin-left:50px;">长X宽X高(单位:米)</font><input class="inp" type="text" id="carlength" placeholder="请输入车辆长宽高" value="<?php echo ($info["vehicleThreeHigh"]); ?>"/>
				</div>
				
				<div class="col-xs-12 pad-up10 font15 bor-b-f2">
					<label>车辆载重</label><input class="inp" type="text" id="carweight" placeholder="请输入车辆载重(单位:吨)" value="<?php echo ($info["vehicleHigh"]); ?>"/>
				</div>
			</div>
			<div class="row bg-fff">
				<div class="col-xs-12 pad-up10 font15">
			...
			<div class="row mar-t15">
				<div class="col-xs-12" id="imageaqa3">
					<button type="button" class="mybtn"> 
						 <?php if($info['vehicleImage'] == '' ): ?><img class="tj" src="/Public/img/tianjia.png"  id="imagess3"/>
					<font size="1" style="margin-left:20px;">图例&nbsp;&nbsp;&nbsp;&nbsp;<img  src="/Public/img/catn.jpg"  class="tj"/></font>
					<?php else: ?>
					<img class="tj" src="<?php echo ($info["vehicleImage"]); ?>"  id="imagess3"/><?php endif; ?>
					</button>
				</div>
			</div>
		...
		$(function(){	
			  $("#goodcheck").click(function(){  
			      ...
			      var reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/; 
			         ...
			         else if(image3=="/Public/img/tianjia.png")
		        	 {
			        	 layer.open({
			        		    content: '请上传图片,再提交申请'
			        		    ,skin: 'msg'
			             ...
			             var url = "<?php echo U('user/dogoodscheck');?>";
			             $.post(url, {username:username,numberid:numberid,phone:phone,carnumber:carnumber
			            	 ,cartype:cartype,carlength:carlength,carweight:carweight,image:image,image1
			            	 :image1,image2:image2,image3:image3}, function(msg){
			             if(msg.state=='200'){
...
//引入Plupload 、qiniu.js后
   var uploader = Qiniu.uploader({
       ...
       runtimes: 'html5,flash,html4',    //上传模式,依次退化
       browse_button: 'imagess3',       //上传选择的点选按钮，**必需**
       uptoken_url: url,            //Ajax请求upToken的Url，**强烈建议设置**（服务端提供）
       //uptoken : tokenQiniu1, //若未指定uptoken_url,则必须指定 uptoken ,uptoken由其他程序生成
       unique_names: true, // 默认 false，key为文件名。若开启该选项，SDK为自动生成上传成功后的key（文件名）。
       save_key: true,   // 默认 false。若在服务端生成uptoken的上传策略中指定了 `sava_key`，则开启，SDK会忽略对key的处理
       domain: 'http://qiniuti.7secai.cn/',   //bucket 域名，下载资源时用到，**必需**
       get_new_uptoken: false,  //设置上传文件的时候是否每次都重新获取新的token
       container: 'imageaqa3',           //上传区域DOM ID，默认是browser_button的父元素，
       max_file_size: '100mb',           //最大文件体积限制
      // flash_swf_url: 'js/plupload/Moxie.swf',  //引入flash,相对路径
       max_retries: 1,                   //上传失败最大重试次数
       dragdrop: false,                   //开启可拖曳上传
       drop_element: 'imageaqa3',        //拖曳上传区域元素的ID，拖曳文件或文件夹后可触发上传
       chunk_size: '4mb',                //分块上传时，每片的体积
       auto_start: true,                 //选择文件后自动上传，若关闭需要自己绑定事件触发上传
      /*  filters : {
           max_file_size : '200kb',
